...
> [feedback: Missing "last message preview" under each session title. The task requires showing the last message content truncated to 60 chars below the session title in the sidebar. To implement: the `getChatMessages` API is too expensive to call per-session. Instead, add a `lastMessagePreview` field to the backend `GET /api/chat/sessions` response — in the LEFT JOIN query, add a correlated subquery or a separate query to get the most recent message content per session, truncated to 60 chars. Then display it in the sidebar item as a second line: `<span className="block truncate text-[10px] text-muted-foreground">{s.lastMessagePreview}</span>` below the title. Update `ChatSessionWithPersona` type in `client.ts` to include `lastMessagePreview?: string`.]
