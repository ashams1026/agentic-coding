...
- [review] **FX.SEC.2** — Fix FTS5 MATCH crash on special characters. `packages/backend/src/routes/search.ts:36,57` passes user input directly to FTS5 MATCH — characters like `AND NOT`, `NEAR(`, unbalanced quotes crash the endpoint with unhandled 500. Wrap queries in try-catch and sanitize/escape FTS5 special syntax before querying.
