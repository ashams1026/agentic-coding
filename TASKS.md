...
- [review] **ACH.6** — Frontend: add chat header bar to the `/chat` page message area. Show persona avatar + name, project badge (or "Global" if no projectId), and editable session title. Double-click title to rename (calls `PATCH /api/chat/sessions/:id`). Add a context menu (three-dot) with: Rename, Delete session.
