...
- [review] **CWF.14** — Frontend: update `packages/frontend/src/features/settings/workflow-config-section.tsx`. Replace hardcoded state list with dynamic states from `useWorkflowStates()`. Add workflow selector dropdown if multiple workflows exist. Persona assignment uses workflow state IDs.
