...
- [review] **ANL.1** — Backend: add `model` (TEXT nullable), `totalTokens` (INTEGER nullable), `toolUses` (INTEGER nullable) columns to `executions` table in `packages/backend/src/db/schema.ts`. Generate migration. Update shared `Execution` entity type. Fix the cents/USD unit inconsistency in `costUsd` column docs (it stores cents, not USD).
