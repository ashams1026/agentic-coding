...
- [review] **OWH.4** — Backend: create `packages/backend/src/routes/webhooks.ts`. CRUD endpoints: `GET /api/webhooks` (list subscriptions), `POST /api/webhooks` (create), `PATCH /api/webhooks/:id` (update url/events/active), `DELETE /api/webhooks/:id`. `GET /api/webhooks/:id/deliveries` (delivery log with status/latency). Register in server.ts.
