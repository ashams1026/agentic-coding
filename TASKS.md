...
- [review] **A.8** — Implement system prompt assembly. In `claude-executor.ts`: build the layered system prompt: (1) persona.systemPrompt (role identity), (2) project context summary (project name, description, key patterns), (3) work item context (title, description, parent chain, inherited context), (4) execution history from executionContext array (previous attempts, rejections). Export `buildSystemPrompt(persona, workItem, project)` function.
