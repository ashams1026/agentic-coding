...
- [review] **PPR.4** — Frontend: add a collapsible "Variables" reference panel below the system prompt textarea in the persona editor. Show all available variables with their current values (use the selected project's data for preview). Add a "Preview" toggle button that switches the textarea to read-only mode showing the fully resolved prompt with variable values highlighted in a distinct color.
