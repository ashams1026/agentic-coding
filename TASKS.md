...
- [review] **DM.2** — Backend: add pre-migration backup hook. In `packages/backend/src/db/migrate.ts`, call `createBackup()` before `runMigrations()`. Log backup path. Skip if no migrations pending.
