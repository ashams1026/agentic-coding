...
- [review] **DM.4** — Backend: add per-table storage stats using `dbstat` virtual table. `GET /api/settings/storage-stats` returns `{ tables: [{ name, rowCount, sizeBytes }], totalSizeBytes }`. Fix existing cascade bug: `DELETE /api/settings/executions` should cascade to proposals/comments.
