...
- [review] **CWF.6** — Backend: update `packages/backend/src/agent/dispatch.ts` to use `resolvePersonaForState(projectId, workflowId, stateName)` with fallback to current `persona_assignments` lookup. Update `packages/backend/src/agent/execution-manager.ts` to set `workflowId` and `workflowStateName` on execution records when creating/updating.
