...
- [review] **CWF.16** — Frontend: create `packages/frontend/src/features/workflow-builder/workflow-preview.tsx` — read-only visual graph of states and transitions. Auto-layout states in a horizontal flow (CSS grid, no graph library in Phase 1). Color-coded state nodes, arrow connections for transitions.
