...
- [review] **ACH.4** — Frontend: create persona selector grid component at `packages/frontend/src/features/pico/persona-selector.tsx`. Display all personas as cards (avatar, name, description, model badge). Highlight Pico as default. On selection, create a new chat session with the chosen `personaId`. Wire into the "New Chat" button on the `/chat` page. If in project scope, pass `projectId`.
