...
- [review] **CWF.11** — Frontend: create `packages/frontend/src/hooks/use-workflows.ts` with React Query hooks: `useWorkflows(projectId?)`, `useWorkflow(id)`, `useWorkflowStates(workflowId)`, `useWorkflowTransitions(workflowId)`. Add API client functions in `packages/frontend/src/api/client.ts`. Cache with 5-minute staleTime.
