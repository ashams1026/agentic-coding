...
- [review] **ANL.6** — Frontend: create `packages/frontend/src/features/analytics/token-usage-tab.tsx`. Render token usage over time (dual-axis: tokens + cost) using Recharts `ComposedChart`. Add breakdown by model pie chart using `PieChart`. Add top N expensive executions table with columns: date, persona, model, tokens, cost, duration.
