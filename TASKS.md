...
- [review] **SDK.HK.4** — Add FileChanged hook for live file tracking. Add a `FileChanged` hook: when an agent modifies a file, emit a WebSocket event `{ type: "file_changed", executionId, filePath, changeType }`. In the agent monitor UI: show a real-time "Files modified" badge/counter on the execution card. Click to expand and see the list of changed files. This gives users live visibility into what an agent is doing to their codebase.
