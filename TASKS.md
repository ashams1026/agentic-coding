...
- [review] **SRC.2** — Backend: create FTS5 sync triggers in a new migration. INSERT/UPDATE/DELETE triggers on each source table that keep the FTS5 tables in sync. Add a one-time backfill script that populates FTS5 from existing data (run at startup if FTS tables are empty).
