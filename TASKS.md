...
- [x] **DM.4** — Backend: add per-table storage stats using `dbstat` virtual table. `GET /api/settings/storage-stats` returns `{ tables: [{ name, rowCount, sizeBytes }], totalSizeBytes }`. Fix existing cascade bug: `DELETE /api/settings/executions` should cascade to proposals/comments. *(completed 2026-04-03 05:30 PDT)*
- [x] **DM.5** — Frontend: create `packages/frontend/src/features/settings/data-management-section.tsx`. Add "Data" tab content in Settings. Backup section: "Create Backup" button, backup list with date/size/restore button. Log truncation: "Truncate old logs" button with day selector. Storage stats: per-table breakdown bar chart or table. *(completed 2026-04-03 05:45 PDT)*
...
- [review] **S27.TEST.4** — Execute Inbound Webhooks + Data Management e2e tests. Screenshot each case. Record results. File bugs as `FX.*`.
