...
- [review] **ANL.5** — Frontend: create `packages/frontend/src/features/analytics/overview-tab.tsx`. Render 4 summary cards (total cost, total executions, success rate, avg duration — reuse existing dashboard stats hook). Add cost trend line chart (7d/30d toggle) using Recharts `LineChart`. Add execution outcomes stacked bar chart using `BarChart`. Add cost-by-persona horizontal bar chart.
