...
- [review] **SDK.V2.4** — Update `docs/architecture.md` with V2 session architecture. Document: the persistent SDK session singleton, its role in discovery and Pico, session lifecycle (startup → ready → reconnect → shutdown), how it relates to per-execution `query()` calls for workflow agents.
