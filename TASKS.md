...
- [review] **FND.GA.8** — Pico scope toggle and persona picker in chat panel header. In `packages/frontend/src/features/pico/pico-store.ts` add `scopeOverride` and `selectedPersonaId` fields. In `chat-panel.tsx` add scope dropdown (Follows sidebar / Global / each project) and persona picker in the header. When either changes, create a fresh session. Display active scope as a badge. Acceptance: user can switch scope and persona; store updates reflected in badge.
