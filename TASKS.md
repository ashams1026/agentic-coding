...
- [x] **OWH.6** — Backend: wire event bus → webhook delivery. When EventBus emits, query matching active subscriptions, create delivery records, trigger worker. Start delivery worker on server startup. *(completed 2026-04-03 03:50 PDT)*

### Inbound Webhooks Phase 1

- [x] **IWH.1** — Backend: add `webhook_triggers` table to schema (id, name, secret, personaId, projectId, promptTemplate TEXT, isActive, createdAt). Add `triggerType` (TEXT nullable) and `triggerId` (TEXT nullable) columns to executions table. Generate migration. *(completed 2026-04-03 04:05 PDT)*
- [x] **IWH.2** — Backend: create `packages/backend/src/routes/webhook-triggers.ts`. Generic receiver: `POST /api/webhooks/trigger/:triggerId` — validate HMAC, extract payload, resolve prompt template (Handlebars-style `{{payload.field}}`), spawn execution with trigger context. CRUD: `GET/POST/PATCH/DELETE /api/webhook-triggers`. Delivery log for inbound. Register in server.ts. *(completed 2026-04-03 04:20 PDT)*
- [x] **IWH.3** — Frontend: add inbound webhook triggers to `integrations-section.tsx`. Trigger list with add/edit/delete. Form: name, persona selector, project selector, prompt template editor with `{{payload.*}}` autocomplete hint, secret display + copy button. *(completed 2026-04-03 04:35 PDT)*

### Data Management Phase 1

- [x] **DM.1** — Backend: create `packages/backend/src/db/backup.ts`. Implement `createBackup()` using SQLite `backup()` API (safe for WAL mode). Store in `~/.agentops/backups/` with timestamped filenames. Implement retention: keep 7 daily + 4 weekly, delete older. Add `restoreBackup(path)` function (stop server, copy file, restart). *(completed 2026-04-03 04:50 PDT)*
