...
- [review] **COL.6** — Frontend: update `packages/frontend/src/features/work-items/detail-panel.tsx` to display handoff notes. In the Execution History section, show a collapsible "Handoff Notes" card for each execution that has them. Display: decisions list, files changed, open questions. Use existing card/badge styling.
