...
- [review] **E.7** — Fix dispatch trigger on state change. Verify that `PATCH /api/work-items/:id` with a `currentState` change actually calls `dispatchForState()`. Verify the dispatch checks persona assignments, concurrency limits, and cost caps. Verify that when auto-routing is ON, the router fires after each persona completes. Verify that when auto-routing is OFF, items stay in their current state after persona completion. Fix any gaps in the wiring between routes → dispatch → execution-manager → router.
