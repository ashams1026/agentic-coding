...
- [review] **PLUG.5** — Publish-ready package.json for shared and core. Update `packages/shared/package.json` and `packages/core/package.json` (from PLUG.3): add proper `main`, `types`, `exports` fields, `files` whitelist, `peerDependencies` where needed. Add `tsconfig.build.json` for declaration-only builds. Ensure `pnpm pack` produces clean tarballs with no dev dependencies. These don't need to be published to npm yet — just structured so they *could* be, and so a sibling project in a monorepo can depend on them cleanly.
