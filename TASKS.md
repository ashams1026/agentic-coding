...
- [x] **O.10** — Build tree view. Create `packages/frontend/src/features/work-items/tree-view.tsx`. Pure hierarchy display — no state grouping. Each node: expand/collapse, title, state badge, priority, progress bar (if has children). Indent levels visually with lines/guides. Click to select → detail panel.
