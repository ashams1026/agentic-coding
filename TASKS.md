...
- [review] **ACH.2** — Backend: update `POST /api/chat/sessions` to persist `personaId` and `workItemId` from request body into the new columns. Update `GET /api/chat/sessions` to join persona data (name, avatar) and return it alongside each session. Update `GET /api/chat/sessions/:id/messages` response to include the session's persona info.
