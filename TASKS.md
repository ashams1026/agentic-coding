...
- [review] **O.14** — Update dashboard for WorkItem model. In dashboard components: replace story/task references with work item. Update `useDashboardStats()` hook to count work items. Update "Upcoming work" widget to show ready work items. Update activity feed events to reference work items.
